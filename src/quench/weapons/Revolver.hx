...
class Revolver extends FlxTypedWeapon<FlxBullet> {
	public function new(parent:FlxSprite) {
		var bulletSize:FlxPoint = FlxPoint.get(16, 16);
		super("revolver", (weapon:FlxWeapon) -> {
		...
		fireRate = 200; // Time between each shot in milliseconds
		...
		setPostFireCallback(() -> {
			...
			currentBullet.camera.shake(0.003, 0.1);
		});
	}
